'use strict';

var hasInformation = require('./hasInformation.cjs');
var whenHasInformation = require('./whenHasInformation.cjs');
var whenHasInformationOtherwise = require('./whenHasInformationOtherwise.cjs');
var whenIsSelected = require('./whenIsSelected.cjs');
var whenIsSelectedOtherwise = require('./whenIsSelectedOtherwise.cjs');
var kind = require('./kind.cjs');
var safeCallback = require('./safeCallback.cjs');
var asyncSafeCallback = require('./asyncSafeCallback.cjs');
var expect = require('./expect.cjs');
var matchInformation = require('./matchInformation.cjs');
var matchInformationOtherwise = require('./matchInformationOtherwise.cjs');
var keepAsRightByInformation = require('./keepAsRightByInformation.cjs');
var keepAsRightSelection = require('./keepAsRightSelection.cjs');
var unwrapByInformation = require('./unwrapByInformation.cjs');
var unwrapByInformationOrThrow = require('./unwrapByInformationOrThrow.cjs');
var unwrapSelection = require('./unwrapSelection.cjs');
var unwrapSelectionOrThrow = require('./unwrapSelectionOrThrow.cjs');
var forwardAssertsSelection = require('./forwardAssertsSelection.cjs');
var create = require('./bool/create.cjs');
var falsy = require('./bool/falsy.cjs');
var falsyOtherwise = require('./bool/falsyOtherwise.cjs');
var truthy = require('./bool/truthy.cjs');
var truthyOtherwise = require('./bool/truthyOtherwise.cjs');
var base = require('./bool/base.cjs');
var create$1 = require('./future/create.cjs');
var error = require('./future/error.cjs');
var success = require('./future/success.cjs');
var base$1 = require('./future/base.cjs');
var create$2 = require('./left/create.cjs');
var error$1 = require('./left/error.cjs');
var fail = require('./left/fail.cjs');
var is = require('./left/is.cjs');
var when = require('./left/when.cjs');
var whenOtherwise = require('./left/whenOtherwise.cjs');
var unwrapOrThrow = require('./left/unwrapOrThrow.cjs');
var unwrap = require('./left/unwrap.cjs');
var create$3 = require('./nullable/create.cjs');
var empty = require('./nullable/empty.cjs');
var emptyOtherwise = require('./nullable/emptyOtherwise.cjs');
var filled = require('./nullable/filled.cjs');
var filledOtherwise = require('./nullable/filledOtherwise.cjs');
var base$2 = require('./nullable/base.cjs');
var create$4 = require('./nullish/create.cjs');
var empty$1 = require('./nullish/empty.cjs');
var emptyOtherwise$1 = require('./nullish/emptyOtherwise.cjs');
var filled$1 = require('./nullish/filled.cjs');
var filledOtherwise$1 = require('./nullish/filledOtherwise.cjs');
var base$3 = require('./nullish/base.cjs');
var create$5 = require('./optional/create.cjs');
var empty$2 = require('./optional/empty.cjs');
var emptyOtherwise$2 = require('./optional/emptyOtherwise.cjs');
var filled$2 = require('./optional/filled.cjs');
var filledOtherwise$2 = require('./optional/filledOtherwise.cjs');
var base$4 = require('./optional/base.cjs');
var asyncPipe = require('./right/asyncPipe.cjs');
var create$6 = require('./right/create.cjs');
var is$1 = require('./right/is.cjs');
var ok = require('./right/ok.cjs');
var pipe = require('./right/pipe.cjs');
var success$1 = require('./right/success.cjs');
var when$1 = require('./right/when.cjs');
var group = require('./right/group.cjs');
var asyncGroup = require('./right/asyncGroup.cjs');
var whenOtherwise$1 = require('./right/whenOtherwise.cjs');
var result = require('./right/result.cjs');
var unwrapOrThrow$1 = require('./right/unwrapOrThrow.cjs');
var unwrap$1 = require('./right/unwrap.cjs');

/**
 * {@include either/index.md}
 */

exports.hasInformation = hasInformation.hasInformation;
exports.whenHasInformation = whenHasInformation.whenHasInformation;
exports.whenHasInformationOtherwise = whenHasInformationOtherwise.whenHasInformationOtherwise;
exports.whenIsSelected = whenIsSelected.whenIsSelected;
exports.whenIsSelectedOtherwise = whenIsSelectedOtherwise.whenIsSelectedOtherwise;
exports.createEitherKind = kind.createEitherKind;
exports.eitherInformationKind = kind.eitherInformationKind;
exports.informationKind = kind.informationKind;
exports.safeCallback = safeCallback.safeCallback;
exports.asyncSafeCallback = asyncSafeCallback.asyncSafeCallback;
exports.expect = expect.expect;
exports.matchInformation = matchInformation.matchInformation;
exports.matchInformationOtherwise = matchInformationOtherwise.matchInformationOtherwise;
exports.keepAsRightByInformation = keepAsRightByInformation.keepAsRightByInformation;
exports.keepAsRightSelection = keepAsRightSelection.keepAsRightSelection;
exports.unwrapByInformation = unwrapByInformation.unwrapByInformation;
exports.HasNotInformationError = unwrapByInformationOrThrow.HasNotInformationError;
exports.unwrapByInformationOrThrow = unwrapByInformationOrThrow.unwrapByInformationOrThrow;
exports.unwrapSelection = unwrapSelection.unwrapSelection;
exports.HasNotSelectedInformationError = unwrapSelectionOrThrow.HasNotSelectedInformationError;
exports.unwrapSelectionOrThrow = unwrapSelectionOrThrow.unwrapSelectionOrThrow;
exports.ForwardAssertsSelectionError = forwardAssertsSelection.ForwardAssertsSelectionError;
exports.forwardAssertsSelection = forwardAssertsSelection.forwardAssertsSelection;
exports.bool = create.bool;
exports.boolFalsy = falsy.boolFalsy;
exports.boolFalsyKind = falsy.boolFalsyKind;
exports.eitherBoolFalsyKind = falsy.eitherBoolFalsyKind;
exports.isBoolFalsy = falsy.isBoolFalsy;
exports.whenIsBoolFalsy = falsy.whenIsBoolFalsy;
exports.whenIsBoolFalsyOtherwise = falsyOtherwise.whenIsBoolFalsyOtherwise;
exports.boolTruthy = truthy.boolTruthy;
exports.boolTruthyKind = truthy.boolTruthyKind;
exports.eitherBoolTruthyKind = truthy.eitherBoolTruthyKind;
exports.isBoolTruthy = truthy.isBoolTruthy;
exports.whenIsBoolTruthy = truthy.whenIsBoolTruthy;
exports.whenIsBoolTruthyOtherwise = truthyOtherwise.whenIsBoolTruthyOtherwise;
exports.boolKind = base.boolKind;
exports.eitherBoolKind = base.eitherBoolKind;
exports.Future = create$1.Future;
exports.future = create$1.future;
exports.eitherFutureErrorKind = error.eitherFutureErrorKind;
exports.futureError = error.futureError;
exports.futureErrorKind = error.futureErrorKind;
exports.eitherFutureSuccessKind = success.eitherFutureSuccessKind;
exports.futureSuccess = success.futureSuccess;
exports.futureSuccessKind = success.futureSuccessKind;
exports.eitherFutureKind = base$1.eitherFutureKind;
exports.futureKind = base$1.futureKind;
exports.eitherLeftKind = create$2.eitherLeftKind;
exports.left = create$2.left;
exports.leftKind = create$2.leftKind;
exports.eitherErrorKind = error$1.eitherErrorKind;
exports.error = error$1.error;
exports.errorKind = error$1.errorKind;
exports.eitherFailKind = fail.eitherFailKind;
exports.fail = fail.fail;
exports.failKind = fail.failKind;
exports.isLeft = is.isLeft;
exports.whenIsLeft = when.whenIsLeft;
exports.whenIsLeftOtherwise = whenOtherwise.whenIsLeftOtherwise;
exports.NotLeftError = unwrapOrThrow.NotLeftError;
exports.unwrapLeftOrThrow = unwrapOrThrow.unwrapLeftOrThrow;
exports.unwrapLeft = unwrap.unwrapLeft;
exports.nullable = create$3.nullable;
exports.eitherNullableEmptyKind = empty.eitherNullableEmptyKind;
exports.isNullableEmpty = empty.isNullableEmpty;
exports.nullableEmpty = empty.nullableEmpty;
exports.nullableEmptyKind = empty.nullableEmptyKind;
exports.whenIsNullableEmpty = empty.whenIsNullableEmpty;
exports.whenIsNullableEmptyOtherwise = emptyOtherwise.whenIsNullableEmptyOtherwise;
exports.eitherNullableFilledKind = filled.eitherNullableFilledKind;
exports.isNullableFilled = filled.isNullableFilled;
exports.nullableFilled = filled.nullableFilled;
exports.nullableFilledKind = filled.nullableFilledKind;
exports.whenIsNullableFilled = filled.whenIsNullableFilled;
exports.whenIsNullableFilledOtherwise = filledOtherwise.whenIsNullableFilledOtherwise;
exports.eitherNullableKind = base$2.eitherNullableKind;
exports.nullableKind = base$2.nullableKind;
exports.nullish = create$4.nullish;
exports.eitherNullishEmptyKind = empty$1.eitherNullishEmptyKind;
exports.isNullishEmpty = empty$1.isNullishEmpty;
exports.nullishEmpty = empty$1.nullishEmpty;
exports.nullishEmptyKind = empty$1.nullishEmptyKind;
exports.whenIsNullishEmpty = empty$1.whenIsNullishEmpty;
exports.whenIsNullishEmptyOtherwise = emptyOtherwise$1.whenIsNullishEmptyOtherwise;
exports.eitherNullishFilledKind = filled$1.eitherNullishFilledKind;
exports.isNullishFilled = filled$1.isNullishFilled;
exports.nullishFilled = filled$1.nullishFilled;
exports.nullishFilledKind = filled$1.nullishFilledKind;
exports.whenIsNullishFilled = filled$1.whenIsNullishFilled;
exports.whenIsNullishFilledOtherwise = filledOtherwise$1.whenIsNullishFilledOtherwise;
exports.eitherNullishKind = base$3.eitherNullishKind;
exports.nullishKind = base$3.nullishKind;
exports.optional = create$5.optional;
exports.eitherOptionalEmptyKind = empty$2.eitherOptionalEmptyKind;
exports.isOptionalEmpty = empty$2.isOptionalEmpty;
exports.optionalEmpty = empty$2.optionalEmpty;
exports.optionalEmptyKind = empty$2.optionalEmptyKind;
exports.whenIsOptionalEmpty = empty$2.whenIsOptionalEmpty;
exports.whenIsOptionalEmptyOtherwise = emptyOtherwise$2.whenIsOptionalEmptyOtherwise;
exports.eitherOptionalFilledKind = filled$2.eitherOptionalFilledKind;
exports.isOptionalFilled = filled$2.isOptionalFilled;
exports.optionalFilled = filled$2.optionalFilled;
exports.optionalFilledKind = filled$2.optionalFilledKind;
exports.whenIsOptionalFilled = filled$2.whenIsOptionalFilled;
exports.whenIsOptionalFilledOtherwise = filledOtherwise$2.whenIsOptionalFilledOtherwise;
exports.eitherOptionalKind = base$4.eitherOptionalKind;
exports.optionalKind = base$4.optionalKind;
exports.rightAsyncPipe = asyncPipe.rightAsyncPipe;
exports.eitherRightKind = create$6.eitherRightKind;
exports.right = create$6.right;
exports.rightKind = create$6.rightKind;
exports.isRight = is$1.isRight;
exports.eitherOkKind = ok.eitherOkKind;
exports.ok = ok.ok;
exports.okKind = ok.okKind;
exports.rightPipe = pipe.rightPipe;
exports.eitherSuccessKind = success$1.eitherSuccessKind;
exports.success = success$1.success;
exports.successKind = success$1.successKind;
exports.whenIsRight = when$1.whenIsRight;
exports.group = group.group;
exports.asyncGroup = asyncGroup.asyncGroup;
exports.whenIsRightOtherwise = whenOtherwise$1.whenIsRightOtherwise;
exports.result = result.result;
exports.resultKind = result.resultKind;
exports.NotRightError = unwrapOrThrow$1.NotRightError;
exports.unwrapRightOrThrow = unwrapOrThrow$1.unwrapRightOrThrow;
exports.unwrapRight = unwrap$1.unwrapRight;
