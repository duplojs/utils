...
function group(group) {
    if (group instanceof Array) {
        return pipe.pipe(group, reduce.reduce(reduce.reduceFrom([]), ({ element, lastValue, nextPush, exit }) => pipe.pipe(element, when.when(isType.isType("function"), (getter) => getter()), when.when(is.isLeft, exit), when$1.whenIsRight((data) => nextPush(lastValue, data)))), whenNot.whenNot(is.isLeft, success.success));
    }
