...
> = (
						...
						& (
							GenericConstructor extends (
								& DCommon.AnyConstructor<[any], (
									& DCommon.Kind<typeof checkerKind.definition>
									& DCommon.Kind<GenericKindHandler["definition"]>
								)>
								& {
									create(...args: never[]):(
								...
								: DCommon.ComputedTypeError<"Function create or execCheck are incorrect.">
						)
					),
				): (
					& GenericConstructor
	...
	& {
		[
		Prop in Exclude<
			keyof typeof DataParserCheckerBase,
			DCommon.ClearClassKeys
		>
		]: typeof DataParserCheckerBase[Prop]
	}
	& {
