...
import { type IsEqual, type And, type Not, type AnyTuple, type Adaptor } from "@scripts/common";
...
type CreateGreaterThanTable<
	GenericSplitReference extends AnyTuple<DString.Digit>,
> = GenericSplitReference extends [
	...
	? InferredRest extends readonly []
		...
		: CreateGreaterThanTable<
			Adaptor<
				InferredRest,
				AnyTuple<DString.Digit>
			>
		> extends infer InferredRestResult extends AnyTuple<FigureGreaterThanTableValue>
			? [FigureGreaterThanTable[InferredFirst], ...InferredRestResult]
			: never
	: never;

type CheckIsGreater<
	GreaterSplitValue extends AnyTuple<DString.Digit>,
	GreaterTableReference extends AnyTuple<FigureGreaterThanTableValue>,
> = GreaterSplitValue[0] extends GreaterTableReference[0][1]
	? true
	: IsEqual<GreaterSplitValue[0], GreaterTableReference[0][0]> extends true
		? IsEqual<GreaterSplitValue["length"], 1> extends true
			? false
			: [
			...
			? [
				...
				? (
					IsEqual<InferredSplitValue["length"], InferredSplitReference["length"]> extends true
						...
						: (
							DArray.CreateTuple<any, InferredSplitValue["length"]> extends [...DArray.CreateTuple<any, InferredSplitReference["length"]>, ...any[]]
							...
							? [
								DArray.JoinTuple<
									Extract<
										IsEqual<InferredLength, InferredSplitValue["length"]> extends true
											? InferredSplitValue
											: DArray.CreateTuple<"0", InferredLength, InferredSplitValue>,
										AnyTuple<string>
									>,
									""
								>,
								DArray.JoinTuple<
									Extract<
										IsEqual<InferredLength, InferredSplitReference["length"]> extends true
											? InferredSplitReference
											: DArray.CreateTuple<"0", InferredLength, InferredSplitReference>,
										AnyTuple<string>
									>,
									""
								>,
				...
				: (
					[
						...
						? IsEqual<InferredSplitValue["length"], InferredSplitReference["length"]> extends true
							...
							>
							: DArray.CreateTuple<0, InferredSplitValue["length"]> extends [
