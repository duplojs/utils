...
describe("whenIsNullableFilledOtherwise", () => {
	it("preserves the normal when callback contract", () => {
		const input = true
			? "value" as const
			: null;
		const result = DEither.whenIsNullableFilledOtherwise(
			input,
			(value) => {
	...
	it("passes the raw input to otherwise", () => {
		const input = null;
		const result = DEither.whenIsNullableFilledOtherwise(input, (value) => value, (value) => {
			type check = ExpectType<
				typeof value,
				null,
				"strict"
	...
	it("supports its curried signature in pipe and keeps non-matching eithers raw", () => {
		const result = pipe(
			...
			DEither.whenIsNullableFilledOtherwise(
				(value) => value,
				(value) => {
					type check = ExpectType<
						typeof value,
